...
; Link: gcc -no-pie -o main main.o glew_wrapper.o -lglfw -lGL -lGLEW
    ...
    extern glColor3f
    extern initGLEW
    extern glfwGetError ; Add this
    extern printf       ; Add this
...
section .data
    ...
    one     dq 1.0
    neg_one dq -1.0
    half_width dq 320.0 ; window_width / 2
...
; pointer tables for glShaderSource (char * const *)
    ...
    error_message db "Error: %s", 10, 0 ; Format string for printf
    error_format db "Error occurred at offset: %d", 10, 0    
    print_format db "Reached line %d", 10, 0  ; Format string for printf
    ok_msg db "glfwInit succeeded\n",0
    err_create db "glfwCreateWindow failed: %s", 10, 0
    err_prefix db "GLFW error: %s", 10, 0
...
.set_hints:
    ...
    ; test rax, rax
    ; jz .glfw_window_failed         ; if NULL → handle failure
    ; mov [window], rax              ; store pointer in our global var
    ; jmp .create_window_ok          ; on success jump to the success label
...
.create_window_ok:
    ...
    ; (Optional) check compile status and print info log if needed
    ; GLint status; glGetShaderiv(shader, GL_COMPILE_STATUS, &status)
    ; For brevity you can add checks later if shader fails.
...
.glfw_window_failed:
    ...
    call glfwGetError         ; int in eax, desc pointer stored at glfw_err_desc
    test eax, eax
    jz .cleanup               ; no description
    mov rdx, [glfw_err_desc]
    mov rdi, err_create
    mov rsi, rdx
    xor eax, eax
